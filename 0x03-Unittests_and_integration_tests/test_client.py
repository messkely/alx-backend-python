...
class TestGithubOrgClient(unittest.TestCase):
    ...
    def test_public_repos(self, mock_get_json):
        ...
        # Mock the _public_repos_url property (a property, so use PropertyMock)
        with patch.object(
            ...
            # Assert mocked property was called
            mock_public_repos_url_property.assert_called_once()
            # Assert get_json was called with fake_repos_url
    ...
    @parameterized.expand(
        [
            ...
            ({}, "my_license", False),  # Repo missing license field
        ]
    )
...
class TestIntegrationGithubOrgClient(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        Called once before tests in this class run.
        cls.org_payload, cls.repos_payload, etc., provided by @parameterized_class.
        """
        if not isinstance(cls.org_payload, dict) or "login" not in cls.org_payload:
            raise ValueError("Fixture 'org_payload' must be a dict and contain 'login' key.")
        if "repos_url" not in cls.org_payload:
            raise ValueError("Fixture 'org_payload' must contain 'repos_url' key.")
        ...
        # Patch 'requests.get' where used by 'get_json' in 'utils.py'
    ...
    def test_public_repos_integration(self):
        """Test GithubOrgClient.public_repos against fixture data."""
