...
import fixtures # Assuming fixtures.py is in the same directory or accessible via PYTHONPATH

# Existing TestGithubOrgClient class from previous tasks
class TestGithubOrgClient(unittest.TestCase):
    ...
    def test_public_repos(self, mock_get_json):
        ...
        # This is a property, so we use PropertyMock
        with patch.object(GithubOrgClient, "_public_repos_url", new_callable=PropertyMock) as mock_public_repos_url_property:
            ...
            # Assert that the mocked property was called
            mock_public_repos_url_property.assert_called_once()
            # Assert that get_json was called with the fake_repos_url
...
# New TestIntegrationGithubOrgClient class for Task 8
@parameterized_class(('org_payload', 'repos_payload', 'expected_repos', 'apache2_repos'), fixtures.TEST_PAYLOAD)
class TestIntegrationGithubOrgClient(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.org_payload, cls.repos_payload, etc., are provided by @parameterized_class.
        """
        # Ensure the org_payload from fixtures contains 'login' and 'repos_url'
        if not isinstance(cls.org_payload, dict) or 'login' not in cls.org_payload:
            raise ValueError("Fixture 'org_payload' must be a dict and contain a 'login' key.")
        if 'repos_url' not in cls.org_payload:
            raise ValueError("Fixture 'org_payload' must contain a 'repos_url' key.")
        ...
        def mock_requests_get_side_effect(url):
            """Side effect function for the mocked requests.get.
            Returns a Mock response object with a json method that returns
            the appropriate fixture based on the URL.
            """
            response_mock = Mock()
            if url == cls.expected_org_url:
                response_mock.json.return_value = cls.org_payload
                # print(f"DEBUG: Mocking GET {url} with org_payload for {org_name}")
            elif url == cls.expected_repos_url:
                response_mock.json.return_value = cls.repos_payload
                # print(f"DEBUG: Mocking GET {url} with repos_payload for {org_name}")
            else:
                # Handle unexpected URLs, e.g., by raising an error or returning a 404-like mock
                response_mock.status_code = 404
                response_mock.json.return_value = {"message": "Not Found - URL not mocked for this test case"}
                # print(f"DEBUG: Unexpected GET {url}")
            return response_mock

        # Start the patcher for 'requests.get' where it's used by 'get_json' in 'utils.py'
    ...
    def tearDownClass(cls):
        ...
        Called once after tests in an individual class have run.
        """
        cls.get_patcher.stop()

    def test_public_repos_integration(self):
        """Test GithubOrgClient.public_repos against expected fixture data."""
        # org_payload is an attribute of the instance, set by @parameterized_class
    ...
    def test_public_repos_with_license_integration(self):
        ...
        against expected fixture data (apache2_repos).
        """
        client = GithubOrgClient(self.org_payload['login'])
        # Assuming the license key for apache2_repos is "apache-2.0"
        # This should match how apache2_repos was generated in fixtures.py
